...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        bestAction = None                                                       # Initialize the best action
        bestDistance = float('inf')                                             # Initialize the shortest distance as infinity

        for dist in livingGhostPositionDistributions:                           # Loop through each ghost's belief distribution
            mostLikelyPos = max(dist.items(), key=lambda x: x[1])[0]            # Find the most likely position of the ghost
            for action in legal:                                                # Loop through all legal actions
                newPos = Actions.getSuccessor(pacmanPosition, action)           # Get the new position after taking the action
                distance = self.distancer.getDistance(newPos, mostLikelyPos)    # Compute the distance to the ghost
                if distance < bestDistance:                                     # Update the best action if a shorter distance is found
                    bestAction = action
                    bestDistance = distance

        return bestAction                                                       # Return the best action found
